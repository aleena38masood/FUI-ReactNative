...
const TeacherLogin = () => {
  ...
  useEffect(() => {
    const handleStoredCredentials = async () => {
      const success = await StoredCredentials(navigation, (error) => {
        setErrors(error);
      });

      if (!success) {
        // If no valid credentials, generate captcha
        generateCaptcha();
      } else {
        // If credentials are found, set global user data
        const userInfo = await AsyncStorage.getItem('userInfo');
        if (userInfo) {
          const userData = JSON.parse(userInfo);
          setGlobal('userName', userData.fullName);
          setGlobal('userRole',userData.isSuperAdmin ? 'SuperAdmin':userData.isAdmin ? 'Admin': userData.isHOD ? 'HOD' : userData.isDean ? 'Dean' : userData.isFaculty ? 'Faculty' : 'Staff');
        }
      }
    };
  ...
  // Always check if decoded.TeacherId exists
  const teacherId = decoded?.TeacherId ? parseInt(decoded.TeacherId, 10) : -1;

  // Save to global state
  setGlobal('teacherId', teacherId);

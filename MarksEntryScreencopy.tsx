...
const totalMarks = parseFloat(editedTotalMarks || (centralDataMarksEntry.tests[courseOfferingID] || []).find(t => t.Test_ID === editingTestId)?.Test_TotalMarks || '0');
 ...
 if (errors.length > 0) {
  ...
  const handleEditTestDetails = (test: Test) => {
    ...
    setEditedSubmissionStartDate(test.Test_Submission_StartDate ? new Date(test.Test_Submission_StartDate) : new Date());
    setEditedSubmissionEndDate(test.Test_Submission_EndDate ? new Date(test.Test_Submission_EndDate) : new Date());
...
const AddQuestion_isDisabled = (() => {
  const totalMarks_addedQuestions = testQuestions.reduce((sum, question) => {
    ...
    if (
      editingQuestion &&
      question.Test_Question_No.toString() === editingQuestion.Test_Question_No.toString()
    ) {
      return sum;
    }
    return sum + parseFloat(question.Test_Question_Marks || "0");
...
console.log("after Set Count "+(presameTypeTests.length+1));
      ...
      style={[styles.inputStyle, { fontSize: isMobile ? 12 : 16 }]}
    >
      ...
      {centralDataMarksEntry.testTypes_Unique.map((testType) => (
        ...
        />
                  ...
                  <Text style={[styles.inputStyle, { fontSize: isMobile ? 12 : 16, padding: 8, borderWidth: 1, borderColor: '#ccc', borderRadius: 4, backgroundColor: '#fff' }]}>
                    {centralDataMarksEntry.testTypes_Unique.find(t => t.testTypeID === (editingTest?.testTypeID || testTypeID))?.testTypeName || 'N/A'}
                  </Text>
                )
              ) : (
  <Text style={{ fontSize: 12, padding: 4 }}>Loading test types...</Text>
)}
...
<TouchableOpacity
          style={[styles.actionButton1, styles.backButton, { backgroundColor: '#4caf50' }]}
          onPress={() => {
        ...
        >
          ...
          <Text style={[styles.buttonText, { marginLeft: 6 }]}>Add Questions</Text>
        </TouchableOpacity>
{formMode !== 'view' && (
            ...
            <View style={[styles.tableContainerLM]}>
              ...
              <View style={styles.form}>
                {/* <View style={styles.formRow_Column}>
                  ...
                  <View style={styles.formRow_Column}>
    <View style={[styles.formControl_MarksEntry, { width: isMobile ? "50%" : "10%" }]}>
      <Text style={[styles.labelStyle]}>Question Number:</Text>
      <Text
        style={[styles.tableCell, { width: isMobile ? "20%" : "20%"}]}
      >
...
const newQuestion: TestQuestions = {
     ...
     disabled={AddQuestion_isDisabled}
                ...
                {testQuestions.length > 0 && (
                  ...
                  <View style={styles.tableWrapper} >
                    <View style={[styles.tableRow, styles.tableHeader]}>
                      <Text style={[styles.tableCell, styles.tableHeaderText, {width: isMobile? "10%": "10%" }]}>Q#</Text>
                      <Text style={[styles.tableCell, styles.tableHeaderText, { width: isMobile? "40%": "40%" }]}>Question Text</Text>
                      <Text style={[styles.tableCell, styles.tableHeaderText, { width: isMobile? "15%": "15%" }]}>Marks</Text>
                      <Text style={[styles.tableCell, styles.tableHeaderText, { width: isMobile? "25%": "20%" }]}>CLO</Text>
                      <Text style={[styles.tableCell, styles.tableHeaderText, { width: isMobile? "10%": "10%" }]}>Actions</Text>
                    </View>
                    {testQuestions.map((q, index) => (
                      <View
                        ...
                        style={[styles.tableRow, index % 2 === 0 ? styles.tableRowEven : styles.tableRowOdd]}
                      >
                        <Text style={[styles.tableCell, { width: '10%' }]}>{q.Test_Question_No}</Text>
                        <Text style={[styles.tableCell, { width: '40%' }]}>{q.Question_Text}</Text>
                        <Text style={[styles.tableCell, { width: '15%' }]}>{q.Test_Question_Marks}</Text>
                        <Text style={[styles.tableCell, { width: '25%' }]}>
                          {centralDataMarksEntry.questionsCLO.find(clo => clo.CLO_ID === q.Test_Question_CLO_ID)?.CLO_Name || 'N/A'}
                        </Text>
                        <View style={[styles.tableCell, { width: '10%', flexDirection: 'row', justifyContent: 'space-around' }]}>
  ...
  // Add state for test details modal and date pickers
const [testDetailsModalVisible, setTestDetailsModalVisible] = useState<boolean>(false);
// const [showEditTestDatePicker, setShowEditTestDatePicker] = useState<boolean>(false);
// const [showEditSubmissionStartPicker, setShowEditSubmissionStartPicker] = useState<boolean>(false);
// const [showEditSubmissionEndPicker, setShowEditSubmissionEndPicker] = useState<boolean>(false);
...
const renderTestSummaryTable = ({ isMobile, summaryData, setShowSummary, images, handleEditStudentMarks, handleEditTestDetails, setSelectedTest, setMarksModalVisible, setModalVisible, styles }) => {
  ...
  return (
    <View style={[styles.tableContainer_MES]}>
      ...
      <View style={styles.tableWrapper}>
        <View style={[styles.tableRow, styles.tableHeader]}>
          <Text style={[styles.tableCell, styles.tableHeaderText, { width: '10%' }]}>S#</Text>
...
<View style={[styles.tableCell, { width: '10%', flexDirection: 'row', justifyContent: 'center' }]}>
                <TooltipWrapper tooltipText="Click to view/edit test details">
                  <TouchableOpacity
                    style={[styles.actionButton1, { marginRight: 0 }]}
                    onPress={() => {
                      ...
                      setEditingTest(test);
                      setEditedTestName(test.Test_Name || '');
                      setEditedTestDate(test.Test_Date ? new Date(test.Test_Date) : new Date());
                      setEditedTotalMarks(test.Test_TotalMarks?.toString() || '');
                      setEditedTestDescription(test.Test_Description || '');
                      setEditedIsVisible(test.Test_IsVisible || false);
                      setEditedSubmissionAllowed(test.Test_SubmissionAllowed || false);
                      setEditedSubmissionStartDate(test.Test_Submission_StartDate ? new Date(test.Test_Submission_StartDate) : new Date());
                      setEditedSubmissionEndDate(test.Test_Submission_EndDate ? new Date(test.Test_Submission_EndDate) : new Date());
                      setTestTypeID(test.TestTypeID || '');
                      const typeName = centralDataMarksEntry.testTypes.find(t => t.TestTypeID === test.TestTypeID)?.TestTypeName || '';
                      setTestTypeName(typeName);
                      if (typeName === 'Exam') {
                        setExamName(test.Test_Name || '');
                      } else {
                        setTestName(test.Test_Name || '');
                      }
                      setTestQuestions(centralDataMarksEntry.testQuestions[test.Test_ID] || []);
                      setQuestionCounter((centralDataMarksEntry.testQuestions[test.Test_ID]?.length || 0) + 1);
...
const renderEditTestForm = () => {
  const validateQuestions = () => {
    ...
    const totalMarks_addedQuestions = testQuestions
      .filter(q => editingQuestion ? q.Test_Question_No.toString() !== editingQuestion.Test_Question_No.toString() : true)
      .reduce((sum, question) => sum + parseFloat(question.Test_Question_Marks || "0"), 0);
    if ((parseFloat(newQuestionMarks || "0") + totalMarks_addedQuestions) > parseFloat(editedTotalMarks || "0")) {
      errors.push(`Sum of question marks cannot exceed total test marks.`);
    }
    if (errors.length > 0) {
      setPopupAlertType('error');
      setPopupAlertTitle('Validation Error');
  ...
  const RadioButton = ({ selected, onPress, label }) => (
    <TouchableOpacity
      style={{ flexDirection: 'row', alignItems: 'center', marginRight: isMobile ? 10 : 15 }}
      onPress={onPress}
    >
      <View
        style={{
  ...
  const availableExamNames = marksSeedLoaded
    ? testTypeName === 'Exam'
      ? examNames.filter(
          (name) =>
            !(centralDataMarksEntry.tests[courseOfferingID] || []).some(
              (test) => test.TestTypeName === 'Exam' && test.Test_Name === name && test.Test_ID !== editingTest?.Test_ID
            )
        )
      : examNames
    : [];
  ...
  const AddQuestion_isDisabled = (() => {
    const totalMarks_addedQuestions = testQuestions.reduce((sum, question) => {
      if (editingQuestion && question.Test_Question_No.toString() === editingQuestion.Test_Question_No.toString()) {
        return sum;
      }
      return sum + parseFloat(question.Test_Question_Marks || "0");
    }, 0);
    return parseFloat(totalMarks_addedQuestions || "0") >= parseFloat(editedTotalMarks || "0");
  })();

  return (
    <View style={[styles.tableContainer_MES]}>
      <View style={styles.lectureHeaderContainer}>
        <Text style={[styles.lectureHeaderText]}>Edit Test</Text>
      </View>
      <View style={styles.form}>
        <View style={styles.formRow}>
          <View style={styles.formControl}>
            <Text style={[styles.labelStyle]}>Test Type:</Text>
            <Text style={[styles.inputStyle, { fontSize: isMobile ? 12 : 16, padding: 8, borderWidth: 1, borderColor: '#ccc', borderRadius: 4, backgroundColor: '#fff' }]}>
              {centralDataMarksEntry.testTypes.find(t => t.TestTypeID === testTypeID)?.TestTypeName || 'N/A'}
            </Text>
          </View>
          <View style={styles.formControl}>
            <Text style={[styles.labelStyle]}>Test Name:</Text>
            {testTypeName === 'Exam' ? (
              <View style={[styles.pickerContainer]}>
                <Picker
                  selectedValue={editedTestName}
                  onValueChange={(value) => setEditedTestName(value)}
                  style={[styles.inputStyle, { fontSize: isMobile ? 12 : 16 }]}
                >
                  <Picker.Item label="Select Exam Name" value="" />
                  {availableExamNames.length === 0 ? (
              ...
              <TextInput
                ...
                onChangeText={setEditedTestName}
          ...
          <View style={styles.formControl}>
            ...
            <TextInput
              ...
              onChangeText={setEditedTotalMarks}
              placeholder="Enter total marks"
              keyboardType="numeric"
              placeholderTextColor="#999"
            />
        ...
        <View style={styles.formRow}>
          <View style={styles.formControl}>
            ...
            <TextInput
              ...
              onChangeText={setEditedTestDescription}
              placeholder="Enter description"
              placeholderTextColor="#999"
            />
          </View>
        ...
        <View style={styles.formRow}>
          <View style={styles.formControl}>
            ...
            <View style={[styles.pickerContainer]}>
              {Platform.OS === 'web' ? (
                <DatePicker
                  selected={getValidDate(editedTestDate)}
                  onChange={(date: Date) => date && !isNaN(date.getTime()) && setEditedTestDate(date)}
                  ...
                  customInput={
                    <TextInput
                      style={[styles.inputStyle, { fontSize: isMobile ? 12 : 16 }]}
                      value={formatDate(editedTestDate)}
                ...
                <>
                  <TouchableOpacity onPress={() => setShowEditTestDatePicker(true)}>
                    <Text style={[styles.inputStyle, { fontSize: isMobile ? 12 : 16 }]}>
                      {formatDate(editedTestDate) || 'Select a date'}
                    </Text>
                  </TouchableOpacity>
                  {showEditTestDatePicker && (
                    <DateTimePicker
                      value={getValidDate(editedTestDate)}
                      mode="date"
                      display={Platform.OS === 'ios' ? 'calendar' : 'default'}
                      onChange={(event, selectedDate) => {
                        setShowEditTestDatePicker(Platform.OS === 'ios');
                        if (selectedDate && !isNaN(selectedDate.getTime())) {
                          setEditedTestDate(selectedDate);
                        }
                      }}
                    />
                  )}
          ...
          {!testTypeName.toLowerCase().includes('exam') && (
            <>
              <View style={styles.formControl}>
                <Text style={[styles.labelStyle]}>Submission Start:</Text>
                <View style={[styles.pickerContainer]}>
                  {Platform.OS === 'web' ? (
                    <DatePicker
                      selected={getValidDate(editedSubmissionStartDate)}
                      onChange={(date: Date) => date && !isNaN(date.getTime()) && setEditedSubmissionStartDate(date)}
                      dateFormat="dd-MMM-yyyy"
                      className="date-picker"
                      wrapperClassName="date-picker-wrapper"
                      placeholderText="Select start date"
                      ...
                      customInput={
                        <TextInput
                          style={[styles.inputStyle, { fontSize: isMobile ? 12 : 16 }]}
                          value={formatDate(editedSubmissionStartDate)}
                    ...
                    <>
                      <TouchableOpacity onPress={() => setShowEditSubmissionStartPicker(true)}>
                        <Text style={[styles.inputStyle, { fontSize: isMobile ? 12 : 16 }]}>
                          {formatDate(editedSubmissionStartDate) || 'Select start date'}
                        </Text>
                      </TouchableOpacity>
                      {showEditSubmissionStartPicker && (
                        <DateTimePicker
                          value={getValidDate(editedSubmissionStartDate)}
                          mode="date"
                          display={Platform.OS === 'ios' ? 'calendar' : 'default'}
                          onChange={(event, selectedDate) => {
                            setShowEditSubmissionStartPicker(Platform.OS === 'ios');
                            if (selectedDate && !isNaN(selectedDate.getTime())) {
                              setEditedSubmissionStartDate(selectedDate);
                            }
                          }}
                        />
                      )}
              ...
              <View style={styles.formControl}>
                ...
                <View style={[styles.pickerContainer]}>
                  {Platform.OS === 'web' ? (
                    <DatePicker
                      selected={getValidDate(editedSubmissionEndDate)}
                      onChange={(date: Date) => date && !isNaN(date.getTime()) && setEditedSubmissionEndDate(date)}
                      dateFormat="dd-MMM-yyyy"
                      className="date-picker"
                      wrapperClassName="date-picker-wrapper"
                      placeholderText="Select end date"
                      ...
                      customInput={
                        <TextInput
                          style={[styles.inputStyle, { fontSize: isMobile ? 12 : 16 }]}
                          value={formatDate(editedSubmissionEndDate)}
                    ...
                    <>
                      <TouchableOpacity onPress={() => setShowEditSubmissionEndPicker(true)}>
                        <Text style={[styles.inputStyle, { fontSize: isMobile ? 12 : 16 }]}>
                          {formatDate(editedSubmissionEndDate) || 'Select end date'}
                        </Text>
                      </TouchableOpacity>
                      {showEditSubmissionEndPicker && (
                        <DateTimePicker
                          value={getValidDate(editedSubmissionEndDate)}
                          mode="date"
                          display={Platform.OS === 'ios' ? 'calendar' : 'default'}
                          onChange={(event, selectedDate) => {
                            setShowEditSubmissionEndPicker(Platform.OS === 'ios');
                            if (selectedDate && !isNaN(selectedDate.getTime())) {
                              setEditedSubmissionEndDate(selectedDate);
                            }
                          }}
                        />
                      )}
        ...
        <View style={styles.formRow}>
          <View style={styles.formControl}>
            <Text style={[styles.labelStyle]}>Visible to Students:</Text>
            <View style={{ flexDirection: 'row' }}>
              <RadioButton
                selected={editedIsVisible}
                onPress={() => setEditedIsVisible(true)}
                label="Yes"
              />
              <RadioButton
                selected={!editedIsVisible}
                onPress={() => setEditedIsVisible(false)}
                label="No"
              />
            </View>
          </View>
          {!testTypeName.toLowerCase().includes('exam') && (
            <View style={styles.formControl}>
              <Text style={[styles.labelStyle]}>Submission Allowed:</Text>
              <View style={{ flexDirection: 'row' }}>
                <RadioButton
                  selected={editedSubmissionAllowed}
                  onPress={() => setEditedSubmissionAllowed(true)}
                  label="Yes"
                />
                <RadioButton
                  selected={!editedSubmissionAllowed}
                  onPress={() => setEditedSubmissionAllowed(false)}
                  label="No"
                />
              </View>
            </View>
        ...
        <View style={styles.formRow}>
          ...
          >
            <Image source={images.arrowLeft} style={styles.icon} />
            <Text style={[styles.buttonText, { fontSize: isMobile ? 12 : 14 }]}>Cancel</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.actionButton1, { backgroundColor: '#4caf50' }]}
            onPress={() => setShowQuestionsForm(!showQuestionsForm)}
          >
            <Image source={images.plusCircle} style={styles.icon} />
            <Text style={[styles.buttonText, { marginLeft: 6, fontSize: isMobile ? 12 : 14 }]}>
              {showQuestionsForm ? 'Hide Questions' : 'Edit Questions'}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.actionButton1, { backgroundColor: '#4caf50' }]}
          ...
          >
            <Text style={[styles.buttonText, { marginRight: 6, fontSize: isMobile ? 12 : 14 }]}>Save</Text>
            <Image source={images.next} style={styles.icon} onError={() => console.warn('Failed to load next icon')} />
          </TouchableOpacity>
        </View>
        {showQuestionsForm && (
          <View style={styles.formRow_Column} ref={questionsTableRef}>
            <View style={[styles.tableContainerLM]}>
              <View style={styles.lectureHeaderContainer}>
                <Text style={[styles.lectureHeaderText]}>Edit Test Questions</Text>
              </View>
              <View style={styles.form}>
                <View style={styles.formRow_Column}>
                  <View style={[styles.formControl_MarksEntry, { width: isMobile ? '50%' : '10%' }]}>
                    <Text style={[styles.labelStyle]}>Question Number:</Text>
                    <Text style={[styles.tableCell, { width: isMobile ? '20%' : '20%' }]}>
                      {editingQuestion ? editingQuestion.Test_Question_No.toString() : questionCounter.toString()} .
                    </Text>
                  </View>
                  <View style={[styles.formControl, { width: isMobile ? '100%' : '50%' }]}>
                    <Text style={[styles.labelStyle]}>Question Text:</Text>
                    <TextInput
                      style={[styles.inputStyle, { width: isMobile ? '90%' : '90%', fontSize: isMobile ? 12 : 16 }]}
                      value={newQuestionText}
                      onChangeText={setNewQuestionText}
                      placeholder="Enter question text"
                      placeholderTextColor="#999"
                  ...
                  <View style={[styles.formControl, { width: isMobile ? '50%' : '10%' }]}>
                    <Text style={[styles.labelStyle]}>Marks:</Text>
                    <TextInput
                      style={[styles.inputStyle, { width: isMobile ? '60%' : '60%', fontSize: isMobile ? 12 : 16 }]}
                      value={newQuestionMarks}
                      onChangeText={setNewQuestionMarks}
                      keyboardType="numeric"
                      placeholder="Enter marks"
                  ...
                  <View style={[styles.formControl, { width: isMobile ? '80%' : '35%' }]}>
                    ...
                    <Picker
                      selectedValue={newQuestionCLO}
                      onValueChange={setNewQuestionCLO}
                      style={[styles.inputStyle, { width: isMobile ? '60%' : '40%', fontSize: isMobile ? 12 : 16 }]}
                    >
                      <Picker.Item label="Select CLO" value="" />
                      {centralDataMarksEntry.questionsCLO.map(clo => (
                        <Picker.Item key={clo.CLO_ID} label={clo.CLO_Name} value={clo.CLO_ID} />
                ...
                <View style={styles.formRow_Column}>
                  <TouchableOpacity
                    style={[styles.actionButton1, { backgroundColor: AddQuestion_isDisabled ? '#ddd' : '#4caf50' }]}
                    onPress={() => {
                      if (editingQuestion) {
                        const { isValid } = validateQuestions();
                        if (!isValid) return;
                        ...
                        const newQuestion: TestQuestions = {
                          ...
                          Test_Question_ID: New_Temp_Question_ID.toString(),
                          Question_Text: newQuestionText,
                          Test_Question_No: questionCounter,
                          Test_Question_Marks: newQuestionMarks,
                          Test_Question_CLO_ID: newQuestionCLO,
                  ...
                  {editingQuestion && (
                    ...
                    >
                      <Text style={[styles.buttonText, { fontSize: isMobile ? 12 : 14 }]}>Cancel Edit</Text>
                    </TouchableOpacity>
                  )}
                </View>
                {testQuestions.length > 0 && (
                    ...
                    <View style={styles.tableWrapper}>
                      ...
                      {testQuestions.map((q, index) => (
                        <View
                          ...
                          style={[styles.tableRow, index % 2 === 0 ? styles.tableRowEven : styles.tableRowOdd]}
                        >
                          <Text style={[styles.tableCell, { width: '10%', fontSize: isMobile ? 12 : 16 }]}>{q.Test_Question_No}</Text>
                          <Text style={[styles.tableCell, { width: '40%', fontSize: isMobile ? 12 : 16 }]}>{q.Question_Text}</Text>
                          <Text style={[styles.tableCell, { width: '15%', fontSize: isMobile ? 12 : 16 }]}>{q.Test_Question_Marks}</Text>
                          <Text style={[styles.tableCell, { width: '25%', fontSize: isMobile ? 12 : 16 }]}>
                            {centralDataMarksEntry.questionsCLO.find(clo => clo.CLO_ID === q.Test_Question_CLO_ID)?.CLO_Name || 'N/A'}
                          </Text>
                          <View style={[styles.tableCell, { width: '10%', flexDirection: 'row', justifyContent: 'space-around' }]}>
                            <TouchableOpacity
                              accessible={true}
                              accessibilityRole="button"
                              style={[styles.actionButtonLM, styles.editButtonLM, { padding: 5 }]}
                              onPress={() => handleEditQuestion(q)}
                            >
                              <Image source={images.pencil} style={[styles.pencilIconLM]} onError={() => console.warn('Failed to load pencil icon')} />
                            </TouchableOpacity>
                            <TouchableOpacity
                              accessible={true}
                              accessibilityRole="button"
                              style={[styles.actionButtonLM, styles.deleteButtonLM, { padding: 5 }]}
                              onPress={() => {
                                setPopupAlertType('confirmation');
...
// New modal: renderTestDetailsModal
const renderTestDetailsModal = () => {
  if (!testDetailsModalVisible) return null;
  ...
  return (
    ...
    >
      <View style={[styles.modalContainer, { zIndex: 1000 }]}>
        <View style={[styles.modalContent, { width: isMobile ? '95%' : '70%', height: isMobile ? '85%' : '80%', overflow: 'scroll' }]}>
          <Text style={[styles.heading, { fontSize: isMobile ? 16 : 18 }]}>Edit Test Details</Text>
          <ScrollView contentContainerStyle={{ paddingBottom: 20 }}>
            {renderEditTestForm()}
          </ScrollView>
        </View>
